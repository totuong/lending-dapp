...
const copyToClipboard = (text: string) => {
    ...
    toast.add({ severity: 'success', summary: 'Copied', detail: 'Copied to clipboard: ' + text, life: 3000 });
};
    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white font-sans flex">
        ...
        <main class="flex-grow p-8 overflow-y-auto">
            ...
            <div class="space-y-6 max-w-4xl">
                ...
                <!-- Protocol Settings -->
                <div
                    class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm dark:shadow-none">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-gray-500 dark:text-gray-400 text-sm mb-1">Lending Pool
                                Address</label>
                            <div
                                class="flex items-center gap-2 bg-gray-100 dark:bg-gray-900 p-3 rounded-lg border border-gray-200 dark:border-gray-700">
                                <code class="text-sm flex-grow truncate">{{ LENDING_POOL_ADDRESS }}</code>
                                <button @click="copyToClipboard(LENDING_POOL_ADDRESS)"
                        ...
                        <div>
                            <label class="block text-gray-500 dark:text-gray-400 text-sm mb-1">Token Address
                                (Mock)</label>
                            <div
                                class="flex items-center gap-2 bg-gray-100 dark:bg-gray-900 p-3 rounded-lg border border-gray-200 dark:border-gray-700">
                                <code class="text-sm flex-grow truncate">{{ MOCK_TOKEN_ADDRESS }}</code>
                                <button @click="copyToClipboard(MOCK_TOKEN_ADDRESS)"
                                    class="text-gray-400 hover:text-white">
                                    <Icon icon="mdi:content-copy" class="w-5 h-5" />
                                </button>
                            </div>
                        ...
                        <div>
                            ...
                            <div
                                class="inline-block bg-green-500/10 text-green-400 border border-green-500/30 px-3 py-1 rounded text-sm font-bold">
                                {{ CHAIN_ID }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <div class="flex gap-2">
                                <button v-for="val in ['0.1', '0.5', '1.0']" :key="val" @click="slippage = val"
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                <label class="block text-gray-600 dark:text-gray-400 font-bold">Infinite
                                    Approval</label>
                                <p class="text-xs text-gray-500">Approve tokens once for minimal transaction fees.</p>
                            </div>
                            <button @click="infiniteApproval = !infiniteApproval"
                                ...
                                <div class="w-4 h-4 bg-white rounded-full shadow-md transform transition-transform duration-300"
                                    :class="infiniteApproval ? 'translate-x-6' : 'translate-x-0'"></div>
                            </button>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <button @click="toggleTheme"
                                ...
                                <Icon :icon="isDark ? 'mdi:weather-night' : 'mdi:weather-sunny'" class="w-5 h-5"
                                    :class="isDark ? 'text-blue-300' : 'text-yellow-400'" />
                            </button>
                        </div>
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex gap-2">
                                <button @click="lang = 'en'"
                                    ...
                                    :class="lang === 'en' ? 'bg-blue-600 text-white border-blue-600' : 'bg-gray-100 dark:bg-gray-900 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:border-blue-400'">
                                    🇺🇸 English
                                </button>
                                <button @click="lang = 'vi'"
                                    class="flex items-center gap-2 px-4 py-2 rounded-lg border transition-all"
                                    :class="lang === 'vi' ? 'bg-blue-600 text-white border-blue-600' : 'bg-gray-100 dark:bg-gray-900 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:border-blue-400'">
                                    🇻🇳 Tiếng Việt
                                </button>
                            </div>
                        </div>
                ...
                <div class="bg-red-500/10 rounded-2xl border border-red-500/30 p-6">
                    ...
                    <p class="text-sm text-red-400 mb-4">
                        Revoking permissions will disable the lending protocol from accessing your assets. You will need
                        to re-approve tokens for future transactions.
                    </p>
